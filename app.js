...
document.getElementById("submit").addEventListener("click", ()=> {
    if(document.querySelector("#chances").value > 0 && document.querySelector("#chances").value < 100){
         vaalue = document.querySelector("#chances").value
            setTimeout(() => {
    ...
    setTimeout(() => {
        ...
        const interval1 = setInterval(() => {
            document.querySelector(".type").textContent = text + " " + count
            count--
        }, 800);
        setTimeout(()=>{
        ...
        setTimeout(() => {
            document.querySelector(".type").textContent = "Every this is ready start the game "
            document.querySelector("#start").style.display = "block"
        }, 4000);
    }, 500);
    }else{
        document.querySelector(".type").textContent = "please enter number and above 0 and below than 100 thank u refresh the page again to restart"
    }
...
document.querySelector("#start").addEventListener("click", ()=>{
    ...
    // document.querySelectorAll(".btn")[2].style.display = "block"
...
for (let i = 0; i < 3; i++) {
        ...
        const computerChoice = choices[rNum];
        let userChoice = who(e);
        let winner = whoWon(userChoice, computerChoice);
        document.querySelector(".txt").textContent = realWinner(winner);
        document.querySelector(".txt").classList.add("blink")
        setTimeout(()=>{
            document.querySelector(".txt").classList.remove("blink")
        },300)
        ...
        if(myScore>compScore){
            document.querySelector(".txt").textContent = "Yooooooo you won "
        }else if(compScore>myScore){
            document.querySelector(".txt").textContent = "damm! no problem try again"
        }else{
...
function who(s) {
    if (s.target == document.querySelectorAll(".btn")[0]) {
        return "rock";
    } else if (s.target == document.querySelectorAll(".btn")[1]) {
        return "paper";
    } else if (s.target == document.querySelectorAll(".btn")[2]) {
        return "scissors";
    }
}
var myScore = 0;
...
function realWinner(result) 
{
    if (result === true) {
        document.querySelector(".txt").style.color = "black"
       setTimeout(() => {
        document.querySelector(".txt").style.color = "#43ffcd"
       }, 300);
        ...
        document.querySelector(".txt").style.color = "black"
        setTimeout(() => {
                    document.querySelector(".txt").style.color = "#ff8000"
        }, 300);
